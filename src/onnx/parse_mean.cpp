...
std::vector<std::size_t> compute_output_lens(std::vector<std::vector<std::size_t>>& input_lens)
{
    ...
    std::transform(input_lens.begin(), input_lens.end(), std::back_inserter(lens_sizes), [&](const auto lens){ return lens.size(); });
...
struct parse_mean : op_parser<parse_mean>
{
                          ...
                          std::vector<instruction_ref> args) const
    {
        ...
        std::transform(args.begin(), args.end(), std::back_inserter(input_lens), [&](const auto arg){ return arg->get_shape().lens(); });
        auto output_lens = compute_output_lens(input_lens);
        auto output_type = args[0]->get_shape().type();

        auto mean = args[0];
        auto divisor = info.add_literal(migraphx::literal{migraphx::shape{mean->get_shape().type()}, {num_data}});
        divisor = info.add_instruction(migraphx::make_op("multibroadcast", {{"out_lens", output_lens}}), divisor);
        if (not std::equal(mean->get_shape().lens().begin(), mean->get_shape().lens().end(), output_lens.begin(), output_lens.end()))
            mean = info.add_instruction(migraphx::make_op("multibroadcast", {{"out_lens", output_lens}}), mean);
        ...
        std::for_each(args.begin() + 1, args.end(), [&](auto& data_i){
            ...
            if (not std::equal(data_i_lens.begin(), data_i_lens.end(), output_lens.begin(), output_lens.end()))
                data_i = info.add_instruction(migraphx::make_op("multibroadcast", {{"out_lens", output_lens}}), data_i);
