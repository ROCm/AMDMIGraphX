...
TEST_CASE(rewrite_pad)
{
    ...
    auto l2 = m.add_instruction(
        migraphx::make_op("pooling", {{"mode", migraphx::op::pooling_mode::kMax}, {"padding", {0, 0, 1, 1}}}), l_img);
...
TEST_CASE(rewrite_pad_symmetric)
{
    ...
    m.add_instruction(
        migraphx::make_op("pooling", {{"mode", migraphx::op::pooling_mode::kMax}, {"padding", {1, 1, 1, 1}}}), l_img);
