...
def filter_data(data, quantile):
    ...
    df['Operation: Line Number'] = df.apply(lambda x: x["Operation"] + ": " + str(x['Line Number']), axis=1)
    df['Cumulative Sum'] = df['Value'].cumsum()

    print(f"Only included the top {round(1-quantile, 2)}% time-intensive operations")
...
def plot_compile_analysis(df, output_path):
    ...
    df_sorted_by_total_time = df.groupby('Operation')['Value'].sum().sort_values(ascending=False).reset_index()
    df_sorted_by_avg_time = df.groupby('Operation')['Value'].mean().sort_values(ascending=False).reset_index()
    ...
    fig = make_subplots(
        ...
        cols=4, 
        row_heights=[0.4, 0.6],
        specs=[[{'type': 'bar'}, {'type': 'bar'}, {'type': 'bar'}, {'type': 'bar'}], [{'type': 'scatter', 'colspan': 4}, None, None, None]],  
        subplot_titles=('Time Taken per Line', 'Total Time Taken per Operation', 'Avg Time Taken per Operation', 'Number of times Operation called', 'Compile Time Series Graph'),
    ...
    fig.add_trace(go.Bar(
        x=df_sorted_by_total_time['Operation'],
    ...
    fig.add_trace(go.Bar(
        x=df_sorted_by_avg_time['Operation'],
