...
struct dnnl_concat : dnnl_extend_op<dnnl_concat, dnnl::concat, op::concat>
{
    std::vector<int> arg_map(int size) const
    {
        ...
        std::iota(result.begin(), result.end(), MIGRAPHX_CPU_BACKEND(ARG_MULTIPLE_SRC));
        return result;
    }
    ...
    desc get_desc(const std::unordered_map<int, dnnl::memory::desc>& m) const
    {
        ...
        {
            srcs.push_back(m.at(MIGRAPHX_CPU_BACKEND(ARG_MULTIPLE_SRC) + i));
        }
        return {m.at(MIGRAPHX_CPU_BACKEND(ARG_DST)), std::size_t(op.axis), srcs};
    }
