...
inline program parse_onnx(const char* filename, migraphx::onnx_options options)
{
...
{
    return program(make<migraphx_program>(&migraphx_parse_onnx_buffer, data, size, options.get_handle_ptr()),
                   own{});
}
