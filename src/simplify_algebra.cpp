...
struct find_slice_reshape_transpose
{
    auto matcher() const
    {
        return slice_reshape_transpose_ops(std::string("slice"), std::string("reshape"));
    }
    ...
    void apply(program& p, match::matcher_result r) const
    {
        ...
        auto r_ins = get_reshape_ins(p, input, dims);
        auto t_ins = get_trans_ins(p, r_ins, perm);
        p.replace_instruction(ins, op::slice{{1}, starts, ends}, t_ins);
    }
};
