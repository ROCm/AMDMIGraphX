...
public:
  explicit NpyFile(const std::string &path) : m_Path(path), m_FStream(m_Path) {
    ...
    // check magic
    CHECK(static_cast<unsigned char>(b[0]) == 0x93 && b[1] == 'N' &&
              b[2] == 'U' && b[3] == 'M' && b[4] == 'P' && b[5] == 'Y',
          "Bad magic: " << m_Path);
    ...
    // parse header
    std::regex re(
        R"re(\{'descr': '[<|][fi]([\d])', 'fortran_order': False, 'shape': \(([\d, ]*)\), \} +\n)re");
    std::smatch matches;
    CHECK(std::regex_match(m_Header, matches, re),
          "Cannot parse numpy header: " << m_Path);
    ...
    std::transform(dims.begin(), dims.end(), m_TensorDims.begin(),
                   [](const std::string &s) { return std::stoi(s); });

    // check header sanity
    size_t tensorSize =
        std::accumulate(m_TensorDims.begin(), m_TensorDims.end(), m_ElementSize,
                        std::multiplies<size_t>());
    CHECK(tensorSize == m_TensorSize,
          "Header description does not match file size: " << m_Path);
    LOG_DEBUG("  Input num=" << m_TensorDims[0] << " | Sample size="
                             << (tensorSize / m_TensorDims[0])
                             << " | Full size=" << m_TensorSize);
  }
  ~NpyFile() { m_FStream.close(); };
  std::string GetPath() const { return m_Path; }
  std::vector<size_t> GetDims() const { return m_TensorDims; }
  size_t GetTensorSize() const { return m_TensorSize; }
  ...
  // load only selected indices from the Tensor, assuming that the first dim is
  // batch dim.
  void LoadSamples(void *dst, const std::vector<size_t> &indices) {
    size_t sampleSize =
        std::accumulate(m_TensorDims.begin() + 1, m_TensorDims.end(),
                        m_ElementSize, std::multiplies<size_t>());
    for (size_t i = 0; i < indices.size(); i++) {
      m_FStream.seekg(10 + m_HeaderSize + indices[i] * sampleSize,
                      std::ios::beg);
