...
struct transform_view {
    ...
    constexpr transform_view(Range& prng, F pf)
      : rng(&prng), f(std::move(pf)) {}

    struct iterator {
        using underlying_iterator = decltype(std::begin(std::declval<Range&>()));
        using reference           = decltype(std::declval<const F>()(std::declval<typename std::iterator_traits<underlying_iterator>::reference>()));
        ...
        struct arrow_proxy {
            reference value;
            constexpr std::add_pointer_t<std::remove_reference_t<reference>> operator->() && { return std::addressof(value); }
        };
        ...
        constexpr reference operator*() const {
            return parent->f(*current);
        }

        constexpr arrow_proxy operator->() const {
            return arrow_proxy{parent->f(*current)};
        }
    ...
    friend constexpr bool operator==(const transform_view& a,
                                     const transform_view& b)
    {
        return std::equal(
            a.begin(), a.end(),
            b.begin(), b.end()
