...
struct find_triaddlayernorm
{
    auto matcher() const
    {
        // auto add1 = match::name("add")(match::args(match::any().bind("z1"), match::any().bind("z2")));
        auto add1 = match::name("add")(match::none_of(match::is_constant()), match::args(match::any().bind("z1"), match::any().bind("z2")));
