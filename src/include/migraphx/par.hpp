...
OutputIt par_transform( InputIt first1, InputIt last1,
                              OutputIt d_first, UnaryOperation unary_op )
{
#ifdef MIGRAPHX_HAS_EXECUTORS
    return std::transform(std::execution::par, first1, last1, d_first, std::move(unary_op));
#else
    simple_par_for(last1 - first1, [&](auto i) {
...
OutputIt par_transform( InputIt1 first1, InputIt1 last1, InputIt2 first2,
                    OutputIt d_first, BinaryOperation binary_op )
{
#ifdef MIGRAPHX_HAS_EXECUTORS
    return std::transform(std::execution::par, first1, last1, first2, d_first, std::move(binary_op));
#else
    simple_par_for(last1 - first1, [&](auto i) {
...
#else
    simple_par_for(last - first, [&](auto i) {
        f(first[i]);
    });
#endif
}
