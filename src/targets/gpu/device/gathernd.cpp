...
gathernd(hipStream_t stream, argument result, argument arg0, argument arg1, const int& batch_dims)
{
    ...
    std::size_t slice_size  = std::accumulate(data_s_lens.begin() + num_slice_dims + batch_dims,
                                             data_s_lens.end(),
                                             1,
                                             std::multiplies<std::size_t>());
    const std::size_t num_batches = std::accumulate(
        data_s_lens.begin(), data_s_lens.begin() + batch_dims, 1, std::multiplies<std::size_t>());
    const std::size_t data_batch_stride = std::accumulate(
        data_s_lens.begin() + batch_dims, data_s_lens.end(), 1, std::multiplies<std::size_t>());
