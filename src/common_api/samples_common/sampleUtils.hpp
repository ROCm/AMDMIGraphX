...
size_t dataTypeSize(mgxinfer1::DataType dataType);
...
int64_t volume(mgxinfer1::Dims const& dims, mgxinfer1::Dims const& strides, int32_t vecDim, int32_t comps, int32_t batch);

using samplesCommon::volume;

mgxinfer1::Dims toDims(std::vector<int32_t> const& vec);
...
template <typename T>
void dumpBuffer(void const* buffer, std::string const& separator, std::ostream& os, mgxinfer1::Dims const& dims,
    mgxinfer1::Dims const& strides, int32_t vectorDim, int32_t spv);
...
void sparsify(mgxinfer1::INetworkDefinition& network, std::vector<std::vector<int8_t>>& sparseWeights);
void sparsify(mgxinfer1::Weights const& weights, int32_t k, int32_t rs, std::vector<int8_t>& sparseWeights);
...
void sparsifyMatMulKernelWeights(
    mgxinfer1::INetworkDefinition& network, std::vector<std::vector<int8_t>>& sparseWeights);
