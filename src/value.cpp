...
{
    object_value_holder() {}
    object_value_holder(std::vector<value> d, std::unordered_map<std::string, std::size_t> l) : data(d), lookup(l)
    {} 
...
value::value(const std::vector<value>& v) : x(nullptr), key()
{
    set_vector(x, v);
}

value::value(const std::string& pkey, const std::vector<value>& v)
: x(nullptr), key(pkey)
{
    set_vector(x, v);
}
...
value::value(const char* i)
: value(std::string(i))
{}

#define MIGRAPHX_VALUE_DEFINE_METHODS(vt, cpp_type) \
    value::value(cpp_type i) : x(std::make_shared<vt ## _value_holder>(i)), key() {} \
    value::value(const std::string& pkey, cpp_type i) : x(std::make_shared<vt ## _value_holder>(i)), key(pkey) {} \
    ...
    const cpp_type* value::if_ ## vt() const \
    { \
        return x ? x->if_ ## vt() : nullptr; \
    } 
MIGRAPHX_VISIT_VALUE_TYPES(MIGRAPHX_VALUE_DEFINE_METHODS)

bool value::is_array() const
{
    return x ? x->get_type() == array_type : false;
}
...
const std::vector<value>* value::if_array() const
{
    return x ? x->if_array() : nullptr;
} 

bool value::is_object() const
{
    return x ? x->get_type() == object_type : false;
}
...
value* value::find(const std::string& pkey)
{
    return find_impl(x, pkey);
}

const value* value::find(const std::string& pkey) const
{
    return find_impl(x, pkey);
}
...
auto compare_common_impl(rank<1>, F f, const std::string& keyx, const T& x, const std::string& keyy, const U& y)
{
    return f(std::forward_as_tuple(keyx, Common(x)), std::forward_as_tuple(keyy, Common(y)));
}

template<class F>
auto compare_common_impl(rank<1>, F f, const std::string& keyx, std::nullptr_t, const std::string& keyy, std::nullptr_t)
{
...
void print_value(std::ostream& os, const std::nullptr_t&)
{
    os << "null";
}
