...
argument hip_roialign::compute(context& ctx, const shape&, const std::vector<argument>& args) const
{
    ...
    return device::roialign(ctx.get_stream().get(), args.back(), in_args, op.coord_trans_mode, op.mode, op.output_height, op.output_width,
                            op.sampling_ratio, op.spatial_scale);
}
