...
struct equality_comparable
{
    ...
    friend constexpr auto operator !=(const X& x, const X& y) { return not (private_ops::equal1(x, y)); }
...
struct less_than_comparable
{
    ...
    friend constexpr bool operator <=(const X& x, const X& y) { return not (private_ops::less1(y, x)); }
    friend constexpr bool operator >=(const X& x, const X& y) { return not (private_ops::less1(x, y)); }
...
struct equivalence
{
    ...
    friend constexpr auto operator==(const X& x, const X& y) { return not private_ops::less1(x, y) and not private_ops::less1(y, x); }

    template <class U, class T>
    friend constexpr auto operator==(const U& x, const T& y) MIGRAPHX_RETURNS(not private_ops::less2(y, x) and not private_ops::greater2(y, x));

    template <class T, class U>
    friend constexpr auto operator==(const T& x, const U& y) MIGRAPHX_RETURNS(not private_ops::less2(x, y) and not private_ops::greater2(x, y));
};
