...
struct Dataset {
  ...
  std::vector<int64_t> getBatchedBuffer(NumpyVector &buffer, size_t value = 0) {
    ...
    for (size_t i = 0; i < batchSize; ++i) {
      ...
      std::copy(buffVec.begin(), buffVec.end(),
                std::back_inserter(batchedBuffer));
    }
...
private:
  ...
  std::string getDatasetPath(const std::string &datasetName) {
    ...
    path << DATASET_FOLDER << datasetName << "_size_"
         << std::to_string(DATASET_SIZE) << "_seq_" << std::to_string(SEQ_SIZE)
         << ".npy";
    return path.str();
  }
  ...
  void prepareSampleDataset() {
    ...
    std::vector<int64_t> input_ids_sample = {1,   6804, 338,  5207,
                                             387, 287,  29973};
