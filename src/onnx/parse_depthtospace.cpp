    ...
    */
        ...
        */
        ...
        if(mode == "DCR")
        {
            ...
            auto temp2 = info.add_instruction(make_op("transpose", {{"permutation", {0, 3, 4, 1, 5, 2}}}), temp1);
            return info.add_instruction(make_op("reshape", {{"dims", lens2}}), info.make_contiguous(temp2));
        ...
        } else if (mode == "CRD")
        {
            ...
            auto temp1 = info.add_instruction(make_op("reshape", {{"dims", lens1}}), args[0]);
            auto temp2 = info.add_instruction(make_op("transpose", {{"permutation", {0, 1, 4, 2, 5, 3}}}), temp1);
            return info.add_instruction(make_op("reshape", {{"dims", lens2}}), info.make_contiguous(temp2));
        } else
