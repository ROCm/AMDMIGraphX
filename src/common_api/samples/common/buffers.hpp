...
class GenericBuffer
{
    ...
    //!
    GenericBuffer(mgxinfer1::DataType type = mgxinfer1::DataType::kFLOAT)
    ...
    //!
    GenericBuffer(size_t size, mgxinfer1::DataType type) : mSize(size), mCapacity(size), mType(type)
    {
        ...
        : mSize(buf.mSize), mCapacity(buf.mCapacity), mType(buf.mType), mBuffer(buf.mBuffer)
    {
        buf.mSize     = 0;
        buf.mCapacity = 0;
        buf.mType     = mgxinfer1::DataType::kFLOAT;
        buf.mBuffer   = nullptr;
    }
    ...
    //!
    void resize(const mgxinfer1::Dims& dims) { return this->resize(samplesCommon::volume(dims)); }
...
{
    public:
    bool operator()(void** ptr, size_t size) const { return hipMalloc(ptr, size) == hipSuccess; }
};
...
class BufferManager
{
    ...
    //!
    BufferManager(std::shared_ptr<mgxinfer1::ICudaEngine> engine,
        ...
        : mEngine(engine), mBatchSize(batchSize)
    {
        ...
        for(int32_t i = 0; i < mEngine->getNbIOTensors(); i++)
        {
            ...
            mgxinfer1::DataType type = mEngine->getTensorDataType(name);
    ...
    BufferManager(std::shared_ptr<mgxinfer1::ICudaEngine> engine,
                  ...
                  mgxinfer1::IExecutionContext const* context = nullptr)
        : mEngine(engine), mBatchSize(batchSize)
    {
        // Create host and device buffers
    ...
    {
        return mEngine->getTensorIOMode(tensorName.c_str()) == mgxinfer1::TensorIOMode::kINPUT;
    }

    void memcpyBuffers(bool const copyInput,
                       bool const deviceToHost,
                       bool const async,
                       hipStream_t const& stream = nullptr)
    {
        for(auto const& n : mNames)
        {
            ...
            const hipMemcpyKind memcpyType =
                deviceToHost ? hipMemcpyDeviceToHost : hipMemcpyHostToDevice;
            if((copyInput && tenosrIsInput(n.first)) || (!copyInput && !tenosrIsInput(n.first)))
            {
                if(async)
                    CHECK(hipMemcpyAsync(dstPtr, srcPtr, byteSize, memcpyType, stream));
                else
                    CHECK(hipMemcpy(dstPtr, srcPtr, byteSize, memcpyType));
            }
        }
    }

    std::shared_ptr<mgxinfer1::ICudaEngine> mEngine; //!< The pointer to the engine
    ...
    std::vector<void*> mDeviceBindings; //!< The vector of device buffers needed for engine
                                        //!< execution
