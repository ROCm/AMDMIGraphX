...
{
    return try_([&] { deref(x).handle = new object_type<T>(f()); });
}

template <class T, class F>
migraphx_status set_object(T* x, F f)
{
    return try_([&] { deref(x).handle = std::addressof(f()); });
}
...
{
    return create_object(dst, [&] { return get_object(target).copy_to(get_object(src)); });
}

extern "C" migraphx_status
migraphx_target_copy_from(migraphx_target target, migraphx_argument src, migraphx_argument* dst)
{
    return create_object(dst, [&] { return get_object(target).copy_from(get_object(src)); });
}
...
MIGRAPHX_DEFINE_OBJECT(migraphx_program_parameter_shapes,
                       std::unordered_map<std::string, migraphx::shape>)

extern "C" migraphx_status migraphx_program_parameter_shapes_create(
...
migraphx_program_parameter_shapes_size(migraphx_program_parameter_shapes program_parameter_shapes,
                                       size_t* size)
{
    return try_([&] { deref(size) = get_object(program_parameter_shapes).size(); });
}
...
extern "C" migraphx_status migraphx_program_parameter_shapes_get_shape(
    migraphx_program_parameter_shapes program_parameter_shapes,
    const char* name,
    migraphx_shape* shape)
{
    return set_object(
        shape, [&]() -> decltype(auto) { return get_object(program_parameter_shapes).at(name); });
}
...
{
    return try_([&] { get_object(program_parameters)[name] = get_object(argument); });
}

extern "C" migraphx_status migraphx_program_run(migraphx_program program,
