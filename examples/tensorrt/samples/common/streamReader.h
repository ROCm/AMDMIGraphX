...
public:
    ...
    int64_t read(void* destination, int64_t nbBytes, hipStream_t stream) noexcept final
    {
        if (!mFile.good())
        {
            return -1;
        ...
        ASSERT(hipPointerGetAttributes(&attributes, destination) == hipSuccess);

        // from CUDA 11 onward, host pointers are return cudaMemoryTypeUnregistered
        if (attributes.type == hipMemoryTypeHost || attributes.type == hipMemoryTypeUnregistered)
        {
        ...
        else if (attributes.type == hipMemoryTypeDevice)
        {
            ...
            // hipMemcpyAsync into device storage.
            ASSERT(hipMemcpyAsync(destination, tmpBuf.get(), nbBytes, hipMemcpyHostToDevice, stream) == hipSuccess);
            // No race between the copying and freeing of tmpBuf, because hipMemcpyAsync will
