...
* `split_single_dyn_dim` now accepts a `bucket_by_optimals` field. When set to `true` and the input `dynamic_dimension::optimals` is non-empty, the pass emits one submodule per optimal value (plus the min/max endpoints) instead of enumerating every integer in `[min, max]`. Collapses compile time and engine size from O(max-min) to O(|optimals|) for wide dynamic ranges. `select_module::compute()` now always falls back to the smallest-compatible bucket (greater-or-equal in all dimensions) when no exact-shape submodule matches, with input zero-padding on the reference backend. The GPU/Ref targets expose the opt-in via the `MIGRAPHX_DYN_DIM_BUCKET_BY_OPTIMALS` env var, read once at compile time and passed as the pass field. See `docs/reference/MIGraphX-dev-env-vars.rst`.
* `select_module::compute()` keeps a per-instance dispatch cache. On a repeat call with the same input-shape signature it bypasses the linear submodule scan and the per-call parameter-name / parameter-shape allocations -- materially reducing runtime dispatch overhead for high-rate inference with frequently repeated input shapes.
