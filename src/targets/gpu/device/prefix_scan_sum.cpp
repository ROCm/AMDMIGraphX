...
void prefix_scan_sum(hipStream_t stream, const argument& result, const argument& arg, int32_t axis)
{
    ...
    rlens[axis] = 1;
    hip_visit_all(result, arg, result.get_shape().with_lens(rlens))([=](auto output, auto input, auto rshape) {
        gs_launch(stream, rshape.elements()*block_size, block_size)([=](auto i, auto idx) __device__ {
            ...
            block_scan<block_size>(idx, sum{}, 0, 
