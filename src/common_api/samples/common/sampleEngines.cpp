...
#include <migraphx/common_api/NvOnnxParser.h>       // #include "NvOnnxParser.h"
...
#include <migraphx/common_api/NvInferConsistency.h>     // #include "NvInferConsistency.h"
#include <migraphx/common_api/safeErrorRecorder.h>      // #include "safeErrorRecorder.h"
#endif
*/
...
public:
    ...
    ~RndInt8Calibrator() override
    {
        ...
        {
            CHECK_WITH_STREAM(hipFree(elem.second), mErr);
        }
    }
    ...
    , mErr(err)
{
    ...
    for (int32_t i = 0; i < network.getNbInputs(); i++)
    {
        ...
        void* data;
        CHECK_WITH_STREAM(hipMalloc(&data, elemCount[i] * sizeof(float)), mErr);
        CHECK_WITH_STREAM(
            hipMemcpy(data, rnd_data.data(), elemCount[i] * sizeof(float), hipMemcpyHostToDevice), mErr);
...
bool printPlanVersion(BuildEnvironment& env, std::ostream& err)
{
    ...
    {
        SMP_RETVAL_IF_FALSE(asyncReader.read(data.data(), kPLAN_SIZE, hipStream_t{}) == kPLAN_SIZE,
