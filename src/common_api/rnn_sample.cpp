...
class SampleCharRNNBase
{
    ...
    protected:
    //!
    //! \brief Add inputs to the TensorRT network and configure LSTM layers using network definition
    //! API.
    //!
...
class SampleCharRNNLoop : public SampleCharRNNBase
{
    ...
    //! API.
    //!
    mgxinfer1::ILayer* addLSTMLayers(SampleUniquePtr<mgxinfer1::INetworkDefinition>& network) final;
...
bool SampleCharRNNBase::build()
{
    ...
    if(mParams.loadEngine.empty())
    {
        ...
        auto network = SampleUniquePtr<mgxinfer1::INetworkDefinition>(builder->createNetworkV2(0));
        if(!network)
        {
            return false;
        }
...
std::map<std::string, mgxinfer1::Weights> SampleCharRNNBase::loadWeights(const std::string file)
{
    ...
    while(count--)
    {
        ...
        // We do not really care about the setup of DataType here. Use char here to avoid additional
        // conversion
        auto mem = new samplesCommon::TypedHostMemory<char, mgxinfer1::DataType::kINT8>(numOfBytes);
    ...
    const auto isolateGate = [&](mgxinfer1::ITensor& gates, int gateIndex) -> mgxinfer1::ITensor* {
        mgxinfer1::ISliceLayer* slice =
            network->addSlice(gates,
                              ...
                              mgxinfer1::Dims{1, {1}});
        return addReshape(network, *slice->getOutput(0), mgxinfer1::Dims{1, {mParams.hiddenSize}});
    };
...
SampleCharRNNLoop::addLSTMLayers(SampleUniquePtr<mgxinfer1::INetworkDefinition>& network)
{
    ...
    mgxinfer1::ITensor* sequenceSize = network->addInput(
        mParams.bindingNames.SEQ_LEN_IN_BLOB_NAME, mgxinfer1::DataType::kINT32, mgxinfer1::Dims{});
...
bool SampleCharRNNBase::infer()
{
    ...
    auto context = SampleUniquePtr<mgxinfer1::IExecutionContext>(mEngine->createExecutionContext());
...
void SampleCharRNNBase::copyEmbeddingToInput(samplesCommon::BufferManager& buffers, char const& c)
{
    ...
    std::memcpy(
        inputBuffer, static_cast<const float*>(embed.values) + index * mParams.dataSize, bufSize);
}
...
{
    std::cout
        << "Usage: ./sample_char_rnn [-h or --help] [-d or --datadir=<path to data directory>]\n";
