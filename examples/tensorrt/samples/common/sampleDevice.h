...
public:
    ...
    ~TrtCudaStream()
    {
        CHECK(hipStreamDestroy(mStream));
    }
    ...
    void synchronize()
    {
        CHECK(hipStreamSynchronize(mStream));
    }
    ...
    void sleep(float* ms)
    {
        CHECK(hipLaunchHostFunc(mStream, cudaSleep, ms));
    }
...
public:
    ...
    {
        const uint32_t flags = blocking ? hipEventBlockingSync : hipEventDefault;
        CHECK(hipEventCreateWithFlags(&mEvent, flags));
    }
    ...
    {
        CHECK(hipEventRecord(mEvent, stream.get()));
    }

    void synchronize()
    {
        CHECK(hipEventSynchronize(mEvent));
    }
    ...
    {
        float time{0};
        CHECK(hipEventElapsedTime(&time, e.get(), get()));
        return time;
    }
...
{
    CHECK(hipStreamWaitEvent(mStream, event.get(), 0));
}
...
public:
    ...
    {
        CHECK(hipStreamBeginCapture(stream.get(), hipStreamCaptureModeThreadLocal));
    }

    bool launch(TrtCudaStream& stream)
    {
        return hipGraphLaunch(mGraphExec, stream.get()) == hipSuccess;
    }

    void endCapture(TrtCudaStream& stream)
    {
        CHECK(hipStreamEndCapture(stream.get(), &mGraph));
        CHECK(hipGraphInstantiate(&mGraphExec, mGraph, nullptr, nullptr, 0));
        CHECK(hipGraphDestroy(mGraph));
    }

    void endCaptureOnError(TrtCudaStream& stream)
    {
        // There are two possibilities why stream capture would fail:
        // (1) stream is in hipErrorStreamCaptureInvalidated state.
        ...
        const auto ret = hipStreamEndCapture(stream.get(), &mGraph);
        if (ret == hipErrorStreamCaptureInvalidated)
        {
...
struct ManagedAllocator
{
    void operator()(void** ptr, size_t size)
    {
        CHECK(hipMallocManaged(ptr, size));
    }
};
...
struct HostAllocator
{
    ...
    //! First attempts to allocate pinned memory using hipHostMalloc(ptr, size), failing that, warns to gLogWarning and
    ...
    void operator()(void** ptr, size_t size)
    {
        ...
        hipError_t ret = hipHostMalloc(ptr, size);
        ...
        if (ret != hipSuccess)
        {
            ...
            sample::gLogWarning << "hipHostMalloc() call with ptr=" << ptr << " and size=" << size
                                << " returns a cuda error: " << hipGetErrorString(ret) << std::endl;
...
struct HostDeallocator
{
    ...
    //! It first checks if ptr is a pinned or pageable host memory. If pinned, call hipHostFree() to free it. If
    ...
    void operator()(void* ptr)
    {
        ...
        CHECK(hipPointerGetAttributes(&attrs, ptr));

        // If pinned, call hipHostFree() to deallocate it.
...
public:
    ...
    {
        CHECK(hipMemcpyAsync(mDeviceBuffer.get(), mHostBuffer.get(), mSize, hipMemcpyHostToDevice, stream.get()));
    }

    void deviceToHost(TrtCudaStream& stream) override
    {
        CHECK(hipMemcpyAsync(mHostBuffer.get(), mDeviceBuffer.get(), mSize, hipMemcpyDeviceToHost, stream.get()));
    }
...
public:
    ...
    void* reallocateOutputAsync(char const* tensorName, void* currentMemory, uint64_t size, uint64_t alignment,
        hipStream_t /*stream*/) noexcept override
    {
