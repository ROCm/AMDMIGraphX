...
#define MIGRAPHX_VISIT_DNNL_ALGO(m)                                                                \
    m(undef) m(convolution_auto) m(convolution_direct) m(convolution_winograd) m(                  \
        deconvolution_direct) m(deconvolution_winograd) m(eltwise_relu) m(eltwise_tanh)            \
        m(eltwise_elu) m(eltwise_square) m(eltwise_abs) m(eltwise_sqrt) m(eltwise_swish) m(        \
            eltwise_linear) m(eltwise_bounded_relu) m(eltwise_soft_relu) m(eltwise_logistic)       \
            m(eltwise_exp) m(eltwise_gelu) m(eltwise_gelu_tanh) m(eltwise_gelu_erf) m(eltwise_log) \
                m(eltwise_clip) m(eltwise_pow) m(eltwise_round) m(eltwise_relu_use_dst_for_bwd) m( \
                    eltwise_tanh_use_dst_for_bwd) m(eltwise_elu_use_dst_for_bwd)                   \
                    m(eltwise_sqrt_use_dst_for_bwd) m(eltwise_logistic_use_dst_for_bwd) m(         \
                        eltwise_exp_use_dst_for_bwd) m(lrn_across_channels) m(lrn_within_channel)  \
                        m(pooling_max) m(pooling_avg) m(pooling_avg_include_padding)               \
                            m(pooling_avg_exclude_padding) m(vanilla_rnn) m(vanilla_lstm)          \
                                m(vanilla_gru) m(lbr_gru) m(binary_add) m(binary_mul)              \
                                    m(binary_max) m(binary_min) m(binary_div)                      \
                                        m(resampling_nearest) m(resampling_linear)                 \
                                            m(reduction_max) m(reduction_min) m(reduction_sum)     \
                                                m(reduction_mul) m(reduction_mean)                 \
