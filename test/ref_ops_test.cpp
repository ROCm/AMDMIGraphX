...
{
    auto create_program = [](const std::string& trans_mode = "half_pixel", const std::string& pooling_mode = "avg") {
        migraphx::program p;
        auto* mm = p.get_main_module();
        migraphx::shape x_s{migraphx::shape::float_type, {1, 1, 10, 10}};
        std::vector<float> x_vec = {
    ...
    {
        auto p = create_program("output_half_pixel", "max");
        p.compile(migraphx::ref::target{});
        auto result = p.eval({}).back();
        std::vector<float> results_vector;
        result.visit([&](auto output) { results_vector.assign(output.begin(), output.end()); });
        std::vector<float> gold = {0.819145, 0.373103, 0.258302, 0.515419, 0.726104, 0.540536, 0.545512, 0.38511, 0.376545, 0.274635, 0.22341, 0.184511, 0.230843, 0.404869, 0.29546, 0.540409, 0.265838, 0.409324, 0.213915, 0.708654, 0.687264, 0.580821, 0.461283, 0.462879, 0.709632, 0.27873, 0.139365, 0.313992, 0.403704, 0.410508, 0.15485, 0.686637, 0.534051, 0.382806, 0.267921, 0.09562, 0.534051, 0.415373, 0.441693, 0.392742, 0.28485, 0.382995, 0.441693, 0.567891, 0.504954, 0.51273, 0.689391, 0.536193, 0.543591, 0.761886, 0.251829, 0.355428, 0.276444, 0.210195, 0.378351, 0.327402, 0.19746, 0.491085, 0.631395, 0.805626, 0.351855, 0.440055, 0.381955, 0.491085, 0.626598, 0.452385, 0.565785, 0.440055, 0.454329, 0.711828, 0.607176, 0.33732, 0.106974, 0.100629, 0.148014};
