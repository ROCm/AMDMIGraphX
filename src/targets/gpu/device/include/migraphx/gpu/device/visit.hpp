...
template<> struct is_hip_type<float> : std::true_type {};
template<> struct is_hip_type<half> : std::true_type {};
template<> struct is_hip_type<bool> : std::true_type {};
template<> struct is_hip_type<std::int8_t> : std::true_type {};
template<> struct is_hip_type<std::uint8_t> : std::true_type {};
...
void hip_visit_all_impl(const shape& s, F f, V&& v, Ts&&... xs)
{
    ...
    visit_tensor_size(s.lens().size(),
                      [&](auto ndim) { s.visit_type(hip_visitor([&](auto as) { v(f(xs, ndim, as)...); })); });
}
