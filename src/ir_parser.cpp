...
struct simple_parser
{
    ...
    void advance(std::size_t n)
    {
        ...
        {
            auto it = std::find_if(
                buffer.begin() + pos, buffer.end(), [](char c) { return !std::isspace(c); });
    ...
    std::match_results<std::string_view::const_iterator> parse_match(const std::regex& re)
    {
        auto rem = peek();
        std::match_results<std::string_view::const_iterator> m;
        if(std::regex_search(rem.begin(), rem.end(), m, re, std::regex_constants::match_continuous))
            advance(m[0].length());
        return m;
    }
    ...
    {
        return "Expected " + std::string(expected) + " at position " + std::to_string(pos) +
...
struct ir_mod_parser
{
    ...
    static shape parse_shape(simple_parser& parser)
    {
        ...
        return shape{
            shape::parse_type(t), dims.to_vector<std::size_t>(), strides.to_vector<std::size_t>()};
    }
            ...
            if(shape::is_integral(s.type()))
            {
                if(shape::is_unsigned(s.type()))
                    return std::make_pair(
                        id, mod->add_literal(literal{s, parse_list(data).to_vector<uint64_t>()}));
                else
                    return std::make_pair(
                        id, mod->add_literal(literal{s, parse_list(data).to_vector<int64_t>()}));
            }
            return std::make_pair(
                id, mod->add_literal(literal{s, parse_list(data).to_vector<double>()}));
        }
