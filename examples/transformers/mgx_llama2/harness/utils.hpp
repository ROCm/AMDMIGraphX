...
static std::string getModelPath(ModelLoadSettings &s) {
  ...
  path << MODEL_FOLDER << "model-" << std::to_string(s.sequnce_length) << "_fp"
       << (s.quantize_fp16 ? "16" : "32") << "_";
...
static migraphx::program loadOnnx(ModelLoadSettings &settings) {
  ...
  if (f.good()) {
    ...
    std::vector<std::size_t> dimsPastKey = {BATCH_SIZE, HIDDEN_LAYERS_NUM,
                                            SEQ_SIZE, HEAD_SIZE};
    ...
    std::cerr << "Onnx file is not available on path: " << onnx_path
              << std::endl;
    exit(1);
  }
  return prog;
...
static migraphx::program create_argmax_program(ModelLoadSettings &settings) {
  ...
  auto argmax_ins =
      m.add_instruction(migraphx::operation("argmax", "{axis: 2}"), {x});
