...
struct miopen_conv_bias_clipped_relu
{
    op::convolution op;
    op::clip clip_op;
    fusion f;
    ...
    std::string name() const { return "gpu::conv_bias_clipped_relu"; }
    shape compute_shape(const std::vector<shape>& inputs) const
    {
        check_shapes{inputs, *this}.has(5);
        // TODO: Check slices
...
void apply_conv_bias_clipped_relu(context& ctx, program& p, match::matcher_result r)
{
    auto conv_ins    = r.instructions["conv"];
    auto bias_ins    = r.instructions["bias"];
    auto ins         = r.result;
    ...
    auto weights_ins = conv_ins->inputs().at(1);
    auto conv_op     = any_cast<miopen_convolution>(conv_ins->get_operator()).op;
    auto clip_op     = any_cast<gpu::hip_clip>(ins->get_operator()).op;
    auto alloc_ins   = ins->inputs().back();
    auto old_ws_ins  = conv_ins->inputs().at(2);
...
struct find_conv_bias_clipped_relu
{
    context* ctx = nullptr;
    auto matcher() const { return match::name("gpu::clipped_relu")(match::arg(0)(conv_bias())); }

    void apply(program& p, match::matcher_result r) const
    {
        apply_conv_bias_clipped_relu(*ctx, p, std::move(r));
    }
};
...
void fuse_ops::apply(program& p) const
{
    ...
    match::find_matches(p, 
        ...
        find_conv_bias_clipped_relu{ctx},
        find_conv_bias{ctx},
        find_add_clip{},
        find_add_relu{}
    );
