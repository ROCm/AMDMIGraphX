...
20. **redundant_static_cast_op.cpp** - Tests `RedundantStaticCastOp`
21. **unnecessary_empty_condition.cpp** - Tests `UnnecessaryEmptyCondition`
22. **use_device_launch.cpp** - Tests `UseDeviceLaunch`
23. **use_manage_pointer.cpp** - Tests `UseManagePointer`
24. **use_smart_pointer.cpp** - Tests `UseSmartPointer`
25. **use_stl_algorithms.cpp** - Tests `useStlAlgorithms`
26. **matcher_nested_parentheses.cpp** - Tests `MatcherNestedParentheses`

### XML Rules Tests (`rules.xml`)

27. **unused_deref.cpp** - Tests `UnusedDeref` rule
28. **strlen_empty_string.cpp** - Tests `StrlenEmptyString` rule
29. **define_rules.cpp** - Tests `defineUpperCase` and `definePrefix` rules
30. **use_named_logic_operator.cpp** - Tests `UseNamedLogicOperator` rules
31. **unnecessary_else_statement.cpp** - Tests `UnnecessaryElseStatement` rule
32. **inverted_logic.cpp** - Tests `InvertedLogic` rule
33. **use_stl_algorithm_loops.cpp** - Tests `useStlAlgorithm` rules for loop patterns
