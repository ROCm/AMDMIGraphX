...
std::string getUuidString(hipUUID uuid)
{
    constexpr int32_t kUUID_SIZE = sizeof(hipUUID);
    static_assert(kUUID_SIZE == 16, "Unexpected size for hipUUID!");
...
void setCudaDevice(int32_t device, std::ostream& os)
{
    ...
    int32_t nbDevices{-1};
    CHECK(hipGetDeviceCount(&nbDevices));
    ...
    {
        hipDeviceProp_t tempProperties;
        CHECK(hipGetDeviceProperties(&tempProperties, deviceIdx));
    ...
    CHECK(hipDeviceGetAttribute(&clockRate, hipDeviceAttributeClockRate, device));
    CHECK(hipDeviceGetAttribute(&memoryClockRate, hipDeviceAttributeMemoryClockRate, device));
...
{
    int32_t version{-1};
    CHECK(hipDriverGetVersion(&version));
    return version;
}
...
{
    int32_t version{-1};
    CHECK(hipRuntimeGetVersion(&version));
    return version;
}
