...
TEST_CASE(test_add_sigmoid) {
    migraphx::program p1; 
    {
        ...
        auto add_buffer = mm->add_instruction(migraphx::make_op("allocate", {{"shape", migraphx::to_value(s)}}));
        auto add = mm->add_instruction(migraphx::make_op("gpu::add"), x, y, add_buffer);
        auto sigmoid_buffer = mm->add_instruction(migraphx::make_op("allocate", {{"shape", migraphx::to_value(s)}}));
    ...
    migraphx::program p2;
    {
        ...
        auto x = mm->add_parameter("x", s);
        auto y = mm->add_parameter("y", s);
        auto buffer = mm->add_instruction(migraphx::make_op("allocate", {{"shape", migraphx::to_value(s)}}));
