...
TEST_CASE(gemm_mutli_dim1_2_3)
{
    ...
    float beta  = 0.41;
    auto m12_alpha =
        migraphx::mul_with_alpha_beta(*mm, std::vector<migraphx::instruction_ref>{l1, l2}, alpha);
...
TEST_CASE(matmul_vv_inner_product)
{
...

    {
        ...
        auto ubl    = mm->add_instruction(migraphx::make_op("unsqueeze", {{"axes", {1}}}), bl);
        float alpha = 0.32f;
        migraphx::mul_with_alpha_beta(*mm, std::vector<migraphx::instruction_ref>{ual, ubl}, alpha);
...
TEST_CASE(matmul_vm)
{
...

    {
        ...
        auto bl     = mm->add_literal(migraphx::literal{b_shape, b});
        float alpha = 0.5f;
        migraphx::mul_with_alpha_beta(*mm, std::vector<migraphx::instruction_ref>{ual, bl}, alpha);
...

    {
        ...
        migraphx::shape b_shape{migraphx::shape::float_type, {3, 6, 4}};
        auto bl = mm->add_literal(migraphx::literal{b_shape, b});
        migraphx::mul_with_alpha_beta(*mm, std::vector<migraphx::instruction_ref>{bual, bl}, 0.21f);
...
TEST_CASE(matmul_mv)
{
...

    {
        ...
        auto ubl    = mm->add_instruction(migraphx::make_op("unsqueeze", {{"axes", {1}}}), bl);
        float alpha = 0.3f;
        migraphx::mul_with_alpha_beta(*mm, std::vector<migraphx::instruction_ref>{al, ubl}, alpha);
