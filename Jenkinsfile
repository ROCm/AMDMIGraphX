...
rocmtest clang_debug: rocmnode('cdna') { cmake_build ->
    stage('hipRTC Debug') {
        ...
        cmake_build(flags: "-DCMAKE_BUILD_TYPE=debug -DMIGRAPHX_ENABLE_PYTHON=Off -DCMAKE_CXX_FLAGS_DEBUG='${debug_flags}' -DCMAKE_C_FLAGS_DEBUG='${debug_flags}' -DMIGRAPHX_USE_HIPRTC=On -DGPU_TARGETS=$(op/rocm/bin/rocminfo | grep -o -m1 'gfx.*')", gpu_debug: true)
    }
}, clang_release: rocmnode('cdna') { cmake_build ->
    stage('Hip Clang Release') {
        cmake_build(flags: "-DCMAKE_BUILD_TYPE=release -DGPU_TARGETS=$(op/rocm/bin/rocminfo | grep -o -m1 'gfx.*')")
...
//     }
    ...
    stage('All targets Release') {
        cmake_build(flags: "-DCMAKE_BUILD_TYPE=release -DMIGRAPHX_ENABLE_GPU=On -DMIGRAPHX_ENABLE_CPU=On -DMIGRAPHX_ENABLE_FPGA=On -DGPU_TARGETS=$(op/rocm/bin/rocminfo | grep -o -m1 'gfx.*')") 
    }
    ...
    stage('MLIR Debug') {
        withEnv(['MIGRAPHX_ENABLE_MLIR=1']) {
            ...
            cmake_build(flags: "-DCMAKE_BUILD_TYPE=debug -DMIGRAPHX_ENABLE_PYTHON=Off -DMIGRAPHX_ENABLE_MLIR=On -DCMAKE_CXX_FLAGS_DEBUG='${debug_flags_cxx}' -DCMAKE_C_FLAGS_DEBUG='${debug_flags}' -DGPU_TARGETS=$(op/rocm/bin/rocminfo | grep -o -m1 'gfx.*')")
        }
    }
