...
for myshape in [[1024, 240], [1024, 64]]:
    ...
    if len(myshape) != len(stride):
        print('Warning!  tensor ', myshape,
              ' doesn\'t have the same number of dimensions as stride ',
              stride, ', no broadcast.')
        stride = {}
    ...
    json_data["tensors"].append({
        "shape": myshape,
        "strides": stride,
        "global_outputs": global_output_item_list
    })
    ...
    for global_workitems in gl_range:
        ...
        for local_workitems_per_CU in range(64, 1025, 64):
            ...
            data_point = ((tensorsize, global_workitems,
                           local_workitems_per_CU, mytime))
            ...
            data_point = {
                ...
                'global': global_workitems,
                'local': local_workitems_per_CU,
                'result, ms': mytime
            }
        ...
        if (len(plot_set) == 0):
            print('Warning: no output for tensor ', myshape,
                  " global items " + str(global_workitems))
            continue

        # dict contents:  sort by key
        dataset = sorted(dataset, key=lambda item: item['tensorsize'])

        global_output_item_dict = {
            'global_workitem_count': global_workitems,
    ...
    save_python_filename = save_python_dir + os.path.splitext(
        os.path.basename(__file__))[0] + '_t' + str(unique_no) + '.py'

    plt.suptitle('time vs local,  ' + remark + '   data shape= ' +
                 json.dumps(myshape) + '   tensor size= ' + str(tensorsize) +
                 '   stride= ' + json.dumps(stride) + '   data=' +
                 str(datatype) + '\nsource: ' + save_python_filename,
                 x=0.5,
                 y=.98,
                 fontsize='small')
    ...
    output_plot_file = os.path.join(
        save_python_dir,
        'plot_' + str(tensorsize) + '_' + str(unique_no) + '.png')
    ...
    print('your results have been plotted as ', output_plot_file,
          '  see also ' + save_python_filename)
...
output_data_file = os.path.join(
    save_python_dir, 'pointwise_perf_results_' + str(unique_no) + '.json')
