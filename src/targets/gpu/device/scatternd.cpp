                   ...
                   const std::string& reduction)
{
    ...
    hip_visit_all(result, arg0, arg2, result.get_shape(), arg2.get_shape())(
        [&](auto output, auto data, auto updates, auto output_shape, auto updates_shape) {
            auto* output_ptr     = device_cast(output.data());
            const auto* data_ptr = device_cast(data.data());
            gs_launch(stream, output_shape.elements())(
                [=](auto i) __device__ { output_ptr[i] = data_ptr[i]; });
