...
#include <migraphx/common_api/NvInferPlugin.h>  // #include "NvInferPlugin.h"
#include "logger.h"
...
#define CHECK_WITH_STREAM(status, stream)                                                                              \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            stream << "Cuda failure at " << __FILE__ << ":" << __LINE__ << ": " << hipGetErrorString(status)          \
                   << std::endl;                                                                                       \
            exit(EXIT_FAILURE);                                                                                        \
        }                                                                                                              \
    } while (0)
...
{
    void* deviceMem;
    CHECK(hipMalloc(&deviceMem, memSize));
...
{
public:
    explicit GpuTimer(hipStream_t stream)
        : mStream(stream)
    {
    ...
    void start() override
    {
        CHECK(hipEventRecord(mStart, mStream));
    }
    void stop() override
    {
        CHECK(hipEventRecord(mStop, mStream));
        float ms{0.0F};
        CHECK(hipEventSynchronize(mStop));
        CHECK(hipEventElapsedTime(&ms, mStart, mStop));
        mMs += ms;
    }
...
struct ComputeCapability
{
    ...
    [[nodiscard]] static ComputeCapability forDevice(int32_t deviceIndex)
    {
        int32_t major{0};
        int32_t minor{0};
        CHECK(hipDeviceGetAttribute(&major, hipDeviceAttributeComputeCapabilityMajor, deviceIndex));
        CHECK(hipDeviceGetAttribute(&minor, hipDeviceAttributeComputeCapabilityMinor, deviceIndex));
...
inline int32_t getSMVersion()
{
    ...
    CHECK(hipGetDevice(&deviceIndex));
...
inline int32_t getMaxPersistentCacheSize()
{
    ...
    CHECK(hipDeviceGetAttribute(&maxPersistentL2CacheSize, cudaDevAttrMaxPersistingL2CacheSize, deviceIndex));
#endif
