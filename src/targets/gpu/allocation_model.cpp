...
gpu_allocation_model::allocation_inserter(module& m) const
{
    ...
    return [=, &m](instruction_ref ins, const shape& s) {
        ...
        {
            auto result =
                m.insert_instruction(ins, make_op("hip::allocate", {{"shape", to_value(s)}}));
            return result;
        }
        ...
        return m.insert_instruction(ins, make_op("hip::allocate", {{"shape", to_value(s)}}));
    };
}
