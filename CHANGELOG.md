...
* Changed out-of-process hipRTC compilation to exchange msgpack over `migraphx-hiprtc-driver`'s stdin and stdout instead of a temporary file, so kernel compilation no longer writes to the filesystem. The driver takes no arguments, replies with an object carrying `code_object`, and reports failure through its exit status. Added `process::read_write`, which writes a child's stdin while concurrently draining its stdout (#5244).
