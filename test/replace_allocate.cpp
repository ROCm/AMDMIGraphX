...
{
    std::string name() const { return "cpu::allocate"; }
    migraphx::operation allocate(const migraphx::shape& s) const { return migraphx::make_op(name(), {{"shape", to_value(s)}}); }
...
{
    migraphx::run_passes(
        m, {migraphx::replace_allocate{model, offload_copy}, migraphx::dead_code_elimination{}});
}
...
migraphx::module create_simple_program()
{
    ...
    auto alloc = m.add_instruction(migraphx::make_op("allocate", {{"shape", migraphx::to_value(s)}}));
...
TEST_CASE(hip_allocate)
{
    ...
    auto alloc2 = m.add_instruction(migraphx::make_op("allocate", {{"shape", migraphx::to_value(s)}}));
