...
struct nonmaxsuppression
{
    ...
    inline bool SuppressByIOU(const float* boxes_data, int64_t box_index1, int64_t box_index2, float iou_threshold) const {
        float x1_min{};
        float y1_min{};
        float x1_max{};
        float y1_max{};
    ...
    argument compute(const shape& output_shape, std::vector<argument> args) const
    {
        ...
        for (std::size_t bidx = 0; bidx < batch_num; ++bidx)
        {
            for(std::size_t cidx = 0; cidx < class_num; ++cidx)
            {
                ...
                int64_t box_idx = 0;
                transform_if(scores.begin() + score_offset, scores.begin() + score_offset + box_num, std::back_inserter(cand_boxes), 
                    [&](auto sc) {
                        box_idx++;
                ...
                std::priority_queue<BoxInfoPtr, std::vector<BoxInfoPtr>> sorted_boxes(std::less<BoxInfoPtr>(), std::move(cand_boxes));
                ...
                while (!sorted_boxes.empty() && static_cast<int64_t>(selected_boxes_inside_class.size()) < max_output_boxes_per_class) {
                    ...
                    for (const auto& selected_index : selected_boxes_inside_class) {
                        if (this->SuppressByIOU(batch_boxes, next_top_score.index_, selected_index.index_, iou_threshold)) {
                            selected = false;
                            break;
                        }
                    }
