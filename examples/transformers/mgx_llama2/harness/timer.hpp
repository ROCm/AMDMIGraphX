...
public:
  ...
  ~Timer() {
    auto total_accum = std::accumulate(
        std::begin(total), std::end(total), 0,
        [](int64_t value,
           std::pair<std::thread::id, std::chrono::duration<double, std::milli>>
               p) { return value + p.second.count(); });

    auto count_accum =
        std::accumulate(std::begin(count), std::end(count), 0,
                        [](size_t value, std::pair<std::thread::id, size_t> p) {
                          return value + p.second;
                        });

    std::cout << "Timer " << tag << " reports "
              << (double)total_accum / count_accum << " ms per call for "
              << count_accum << " times." << std::endl;
    if (verbose) {
...
private:
  std::string tag;
  bool verbose;
  std::unordered_map<std::thread::id, std::chrono::duration<double, std::milli>>
      total;
