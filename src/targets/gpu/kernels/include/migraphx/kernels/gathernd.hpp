...
__device__ void
gathernd(const T& data_t, const U& indices_t, const V& output_t, Settings s)
{
    auto ind           = make_index();
    auto i               = ind.global;
    ...
    std::size_t slice_size  = accumulate(data_shape_lens.begin() + num_slice_dims + batch_dims,
                                             ...
                                             1,
                                             std::multiplies<std::size_t>());
    const std::size_t num_batches = accumulate(
        data_shape_lens.begin(), data_shape_lens.begin() + batch_dims, 1, std::multiplies<std::size_t>());
    const std::size_t data_batch_stride = accumulate(
        data_shape_lens.begin() + batch_dims, data_shape_lens.end(), 1, std::multiplies<std::size_t>());
    ...
    if(i < output_shape.elements())
    {
        ...
        for (std::size_t idx = 0; idx < num_slice_dims; ++idx)
        {
            ...
            std::size_t size_from_slice_dims = accumulate(data_shape_lens.begin() + batch_dims + idx + 1, data_shape_lens.begin() + batch_dims + num_slice_dims, slice_size, std::multiplies<std::size_t>());
