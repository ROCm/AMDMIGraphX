...
constexpr int countl_zero(T x) noexcept { return __builtin_clzg(x, numeric_limits<T>::digits); }

template<class T, ROCM_REQUIRES(rocm::is_unsigned<T>{})>
constexpr int countl_one(T x) noexcept { return countl_zero(T(~x)); }

template<class T, ROCM_REQUIRES(rocm::is_unsigned<T>{})>
constexpr int countr_zero(T x) noexcept { return __builtin_ctzg(x, numeric_limits<T>::digits); }

template<class T, ROCM_REQUIRES(rocm::is_unsigned<T>{})>
constexpr int countr_one(T x) noexcept { return countr_zero(T(~x)); }

template<class T, ROCM_REQUIRES(rocm::is_unsigned<T>{})>
constexpr int popcount(T x) noexcept { return __builtin_popcountg(x); }

template<class T, ROCM_REQUIRES(rocm::is_unsigned<T>{})>
constexpr int bit_width(T x) noexcept { return numeric_limits<T>::digits - countl_zero(x); }
...
template<class T, ROCM_REQUIRES(rocm::is_unsigned<T>{})>
constexpr bool has_single_bit(T x) noexcept { return popcount(x) == 1; }
