...
{
    static std::unordered_map<std::string, std::string> m = {{"convolution", "quant_convolution"},
                                                             {"dot", "quant_dot"}};
    return m;
}
...
std::unordered_set<std::string> get_quantized_operators()
{
    ...
    std::transform(quant_ops_map().begin(),
                   quant_ops_map().end(),
...
struct match_find_dqopq
{
    ...
    void apply(module& mod, match::matcher_result r) const
    {
        ...
        if(replace and not mismatch)
        {
            ...
            {
                mod.replace_instruction(
                    ins, migraphx::make_op(quant_ops_map().at(ins->name())), args);
            }
            std::cout << ins->get_operator() << ", " << ins->get_shape() << std::endl;
            auto dq_args    = dq->inputs();
            dq_args.front() = ins;
...
{
    auto matcher() const { return ;//match::any()(match::either_arg(0,
1)(match::name("dequantizelinear"), match::name("dequantizelinear"))); } void apply(module& mod,
match::matcher_result r) const
    {
        auto res = r.result;
