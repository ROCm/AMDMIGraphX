...
struct Numa final {
  ...
  explicit Numa(const NumaSettings &numa_settings)
      : numa_settings{numa_settings} {}

  inline bool UseNuma() const { return not numa_settings.numa_config.empty(); }
...
inline void bindThreadToCpus(std::thread &th, const std::vector<size_t> &cpus,
                             const bool ignore_esrch = false) {
  cpu_set_t cpuset;
  CPU_ZERO(&cpuset);
  for (int cpu : cpus) {
    CPU_SET(cpu, &cpuset);
...
inline std::vector<size_t> parseRange(const std::string &s) {
  ...
  for (const auto &range : ranges) {
    ...
    CHECK((startEnd.size() <= 2),
          "Invalid numa_config setting. Expects zero or one '-'.");
    if (startEnd.size() == 1) {
...
// Example of the format: "0,2:0-63&1,3:64-127" for 4 GPUs, 128 CPU, 2 NUMA node
// system.
inline NumaConfig parseNumaConfig(const std::string &numa_file) {
  ...
  if (!numa_str.empty()) {
    ...
    for (const auto &node : nodes) {
      ...
      CHECK((pair.size() == 2),
            "Invalid numa_config setting. Expects one ':'.");
