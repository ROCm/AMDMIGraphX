...
void apply_quantizelinear(module& m, instruction_ref ins)
{
    ...
    add_zero_point = m.insert_instruction(ins, make_op("convert", {{"target_type", shape::int32_type}}), add_zero_point);
    if(ins->inputs().size() ==3)
    {
        auto zero_point = ins->inputs()[2];
        zero_point = m.insert_instruction(ins, make_op("convert", {{"target_type", shape::int32_type}}), zero_point);
...
void apply_dequantizelinear(module& m, instruction_ref ins)
{
    ...
    if(ins->inputs().size() == 3)
    {
        ...
        auto zero_point_int32 = m.insert_instruction(ins, make_op("convert", {{"target_type", shape::int32_type}}), x_zero_point);
        auto sub_zp_int32 =
        ...
        dequant_input = m.insert_instruction(ins, make_op("convert", {{"target_type", shape::float_type}}), sub);
    }
    else 
    {
        dequant_input = m.insert_instruction(ins, make_op("convert", {{"target_type", shape::float_type}}), x);
    }
