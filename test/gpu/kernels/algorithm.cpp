...
TEST_CASE(accumulate_empty_range) {
    empty_range arr = {};
    auto sum = migraphx::accumulate(arr.begin(), arr.begin(), 42, [](int a, int b) { return a + b; });
    EXPECT(sum == 42);
}
TEST_CASE(accumulate_multiply) {
    migraphx::array<int, 3> arr = {2, 3, 4};
    auto product = migraphx::accumulate(arr.begin(), arr.end(), 1, [](int a, int b) { return a * b; });
    EXPECT(product == 24);
}
...
TEST_CASE(copy_if_basic) {
    ...
    auto end_it = migraphx::copy_if(src.begin(), src.end(), dst.begin(), [](int x) { return x % 2 == 0; });
    EXPECT(dst[0] == 2);
    EXPECT(dst[1] == 4);
    EXPECT(dst[2] == 6);
...
TEST_CASE(search_partial_range) {
    ...
    auto result = migraphx::search(haystack.begin() + 2, haystack.begin() + 7, needle.begin(), needle.end());
    EXPECT(result == haystack.begin() + 4);
}
...
TEST_CASE(merge_basic) {
    ...
    migraphx::merge(arr1.begin(), arr1.end(), arr2.begin(), arr2.end(), result.begin(), migraphx::less{});
...
TEST_CASE(merge_both_empty) {
    ...
    empty_range arr2 = {};
    empty_range result = {};
    auto end_it = migraphx::merge(arr1.begin(), arr1.end(), arr2.begin(), arr2.end(), result.begin(), migraphx::less{});
...
TEST_CASE(merge_reverse_order) {
    ...
    migraphx::array<int, 6> result = {0, 0, 0, 0, 0, 0};
    migraphx::merge(arr1.begin(), arr1.end(), arr2.begin(), arr2.end(), result.begin(), migraphx::greater{});
...
TEST_CASE(merge_partial_ranges) {
    ...
    migraphx::array<int, 5> result = {0, 0, 0, 0, 0};
    migraphx::merge(arr1.begin() + 2, arr1.begin() + 5, arr2.begin() + 1, arr2.begin() + 3, result.begin(), migraphx::less{});
...
TEST_CASE(common_mistakes_predicate_consistency) {
    migraphx::array<int, 4> arr = {1, 2, 3, 4};
    int call_count = 0;
    auto predicate = [&call_count](int x) { call_count++; return x > 2; };
