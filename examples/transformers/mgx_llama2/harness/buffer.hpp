...
struct GenericBuffer : public IBuffer<AllocFunc, FreeFunc> {
  GenericBuffer() : size_in_bytes{0}, stride_in_bytes{0}, tensor_ptr{nullptr} {}
  ...
  GenericBuffer(GenericBuffer &&buf)
      : size_in_bytes{buf.size_in_bytes}, stride_in_bytes{buf.stride_in_bytes},
...
template <typename T> struct ManagedBuffer_v2 {
  ...
  void *data() { return dbuff.tensor_ptr; }

  void update(std::vector<T> &&host_data) { hbuff = std::move(host_data); }

  void upload_to_device(hipStream_t stream, size_t start_idx = 0,
                        size_t end_idx = 0) {
    ...
    check_hip_status(
        hipMemcpyHtoDAsync(dst_addr, src_addr, copy_size_in_bytes, stream));
  }

  void download_from_device(hipStream_t stream, size_t start_idx = 0,
                            size_t end_idx = 0) {
    ...
    check_hip_status(
        hipMemcpyDtoHAsync(dst_addr, src_addr, copy_size_in_bytes, stream));
  }
  ...
  void update_data(T data, size_t position, hipStream_t stream) {
    ...
    // check_hip_status(hipMemcpy(get_device_ptr<void*>(),
    // get_host_ptr<void*>(), dbuff.size_in_bytes,
