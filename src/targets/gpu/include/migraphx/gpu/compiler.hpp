...
using compiler_compile_op = std::function<operation(context&, const std::vector<shape>& inputs, const value&)>;
...
{
    T c;
    register_compiler(c.name(), [=](auto&&... xs) { return c.compile(std::forward<decltype(xs)>(xs)...); },
