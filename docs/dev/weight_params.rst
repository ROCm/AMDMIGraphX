.. meta::
  :description: MIGraphX external weights (MXR weight encoding) reference
  :keywords: MIGraphX, external weights, weight encoding, MXR, external_weight, replace_onnx_external_weights

.. _weight-params:

External Weights (MXR Weight Encoding)
====================================
...
ONNX parser reads those files and encodes the values directly into the program as
...
4. **Encode a weight set** -- :cpp:func:`replace_onnx_external_weights` reads raw
   weight bytes from a directory and produces a new, self-contained program.
5. **Save the encoded program as an MXR** -- deploy a model with the weights
...
weight sets can be encoded from one compiled template without any recompilation.
...
.. code-block:: text
    ...
    program.compile(target)                           # compile template once
                   |
                   v
    replace_onnx_external_weights(prog, dir, target)       # external_weight -> encoded @literal
                   |                                     (per weight set)
                   v
    save(encoded, "model.mxr")                          # serialize self-contained MXR
                   |
                   v
...
shape. :cpp:func:`replace_onnx_external_weights` later walks the IR (including
...
.. note::
   ...
   literals. ``replace_onnx_external_weights`` is a no-op on programs that contain no
   ``external_weight`` ops.
...
.. code-block:: cpp
    ...
    migraphx::program encoded =
        migraphx::replace_onnx_external_weights(prog, "weights_v1", t);

    // Persist the self-contained result.
    migraphx::file_options fo;
    migraphx::save(encoded, "model_v1.mxr", fo);

The free function is declared in ``migraphx/onnx.hpp`` (the C/C++/Python API
wrappers expose it under the shorter name ``replace_onnx_external_weights``):
...
``replace_onnx_external_weights`` deliberately does **not** finalize the program.
...
The encoded program therefore is not yet runnable on the device. The portable way
to make it runnable is to save it and load it back: loading a compiled MXR
finalizes it automatically, allocating device buffers and uploading the encoded
literals.

.. code-block:: cpp

    auto encoded = migraphx::replace_onnx_external_weights(prog, "weights_v1", t);
    migraphx::save(encoded, "model_v1.mxr");        // serialize the encoded program
...
.. note::
   ...
   that finalizes a encoded or loaded program in place, but it is **not** exposed
...
.. code-block:: python
    ...
    # 3. Encode a weight set from a directory of raw .bin files.
    encoded = migraphx.replace_onnx_external_weights(prog, "weights_v1", gpu)

    # 4. Save the self-contained MXR for later deployment.
    migraphx.save(encoded, "model_v1.mxr")

    # 5. Run in-process. Encoding does not finalize, so materialize the device
    #    buffers first. Python exposes program.finalize() directly...
    encoded.finalize(gpu)
    # ...or, equivalently and portably, reload the saved MXR which finalizes
    #    on load:  runnable = migraphx.load("model_v1.mxr")

    run_params = {}
    for name, shape in encoded.get_parameter_shapes().items():
        if name == "input":
...
``replace_onnx_external_weights(program, base_dir, target)`` mirrors the C++
function above. Unlike the C/C++ wrappers, the Python bindings additionally
expose ``program.finalize(t)``, which finalizes a encoded program in place so it
...
The ``compile`` command can also encode a weight set in the same step with
``--encode-weights <dir>``. The input may be the ONNX model directly or a
...
.. code-block:: bash
    ...
    # Encode straight from the ONNX model.
    migraphx-driver compile model.onnx --weight-params --gpu \
        --encode-weights weights_v1 -o model_v1.mxr
...
encodes the weights from ``<dir>``, and saves the result. ``--encode-weights``
expects that the program was parsed with ``--weight-params`` (so it contains
``external_weight`` ops); otherwise there is nothing to encode.

Weight Directory Layout
-----------------------

``replace_onnx_external_weights`` reads each weight from ``base_dir`` using the
...
bytes laid out exactly as the model expects. To encode a different weight set,
...
* ``replace_onnx_external_weights`` returns a copy, leaving the template untouched so
  it can be reused for the next weight set. When the program is compiled, the
  target's lowering passes are applied to the encoded literals.
...
* Encoding does not finalize. To run a encoded program in-process, save it and load
