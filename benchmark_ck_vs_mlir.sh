...
print_stats() {
    ...
    NR > 1 {
        mm = matches($3, fm); mn = matches($4, fn); mk = matches($5, fk); mo = matches($6, fo)
        if (fmode == "and") {
            if (!mm || !mn || !mk || !mo) next
        } else {
            hit = 0
        ...
        if ($9 == "ck") {
            ...
            ck_sum += $11
            if (ck_count == 1 || $11 < ck_min) ck_min = $11
            if (ck_count == 1 || $11 > ck_max) ck_max = $11
        } else {
            mlir_count++
            mlir_speedups[mlir_count] = -$11
            mlir_sum += -$11
            if (mlir_count == 1 || -$11 < mlir_min) mlir_min = -$11
            if (mlir_count == 1 || -$11 > mlir_max) mlir_max = -$11
        }
    }
