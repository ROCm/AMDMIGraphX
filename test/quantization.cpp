...
TEST_CASE(dot_float)
{
    auto create_program = [] {
        ...
        auto r = migraphx::mul_with_alpha_beta(*mm, {pa, pb});
        mm->add_return({r});
    ...
    auto create_int8_quantized_prog = [] {
        ...
        auto r = migraphx::mul_with_alpha_beta(*mm, {dqa, dqb});
        mm->add_return({r});
...
TEST_CASE(dot_half_1arg)
{
    auto create_program = [] {
        ...
        auto r = migraphx::mul_with_alpha_beta(*mm, {x, x});
        mm->add_return({r});
...
TEST_CASE(int8_subgraph)
{
    auto create_program = [] {
        ...
        auto* then_mod = p.create_module("If_6_if");
        auto out1      = migraphx::mul_with_alpha_beta(*then_mod, {a, b});
        then_mod->add_return({out1});
