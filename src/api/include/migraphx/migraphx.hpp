...
struct name : handle_base<migraphx_ ## name, decltype(&migraphx_ ## name ## _destroy), migraphx_ ## name ## _destroy>

MIGRAPHX_HANDLE(shape)
{
    shape()
    : m_handle(nullptr)
    {}
    ...
    shape(std::vector<size_t> plengths)
    : m_handle(nullptr)
    {
...
MIGRAPHX_HANDLE(argument)
{
    ...
    : m_handle(nullptr)
    {}
    argument(migraphx_argument p, bool own = true)
    : m_handle(nullptr)
    {
        this->set_handle(p, own);
    }
...
MIGRAPHX_HANDLE(target)
{
    ...
    : m_handle(nullptr)
    {}
    target(migraphx_target p, bool own = true)
    : m_handle(nullptr)
    {
        this->set_handle(p, own);
    }
