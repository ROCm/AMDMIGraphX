...
struct unfold_range {
    unfold_range(State pz, F pf, G pg)
...
/**
 * @brief Returns a range which generates a sequence by repeatedly applying a step function and projecting each state.
 *
 * The `unfold` function creates a range starting from an initial state `z`. At each step, the range yields `f(state)` as the sequence element,
 * and advances the state by applying `g(state)`. If `g(state)` returns `std::nullopt`, the sequence ends.
 *
 * - The range's value type is the result of `f(State)`.
 * - The sequence begins with the initial state `z`.
 * - At each iteration, the current element is `f(state)`, and the next state is computed by `g(state)`.
 * - When `g(state)` returns `std::nullopt`, the range ends (the end iterator is reached).
 *
 * This function is analogous to Haskell's `unfoldr`, but instead of returning a pair `(a, b)` or `Nothing`, the generator function `g` here is
 * only responsible for producing the next state: on each step, `f(state)` produces the value, and `g(state)` produces the next state or `std::nullopt` to stop.
 *
 * Example:
 ...
 * @param g Function to step to the next state. Returns std::optional<State>; if std::nullopt is returned, iteration stops.
