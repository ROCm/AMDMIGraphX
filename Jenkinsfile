...
def rocmnodename(name) {
    ...
    } else if(name == "rocm-framework-19.amd.com") {
        node_name = "${rocmtest_name} && rocm-framework-19.amd.com";
    } else if(name == "nogpu") {
...
}, mlir_debug: rocmnode('rocm-framework-19.amd.com') { cmake_build ->
    stage('MLIR Debug') {
        def sanitizers = "undefined"
        def debug_flags = "-g -O2 -fsanitize=${sanitizers} -fno-sanitize-recover=${sanitizers}"
...
}, clang_asan: rocmnode('rocm-framework-19.amd.com') { cmake_build ->
    stage('Clang ASAN') {
