        ...
        if (ins->op.name() == "batch_norm_inference") {
            ...
            if (ins_prev->op.name() == "convolution") {
                ...
                visit_all(weights, gamma, bias, mean, variance, new_weights, new_bias)(
                    [&](auto weights2, auto gamma2, auto bias2, auto mean2, 
                        auto variance2, auto new_weights2, auto new_bias2) {
                    dfor(out_channels, in_channels, height, width)(
                        [&](std::size_t k, std::size_t c, std::size_t h, std::size_t w) {
                        new_weights2(k, c, h, w) = gamma2(k) / std::sqrt(variance2(k)+epsilon) * weights2(k, c, h, w); 
                        new_bias2(k, c, h, w) = bias2(k) - (mean2(k) / std::sqrt(variance2(k)+epsilon)); 
                    });
                });
                ...
                auto c = p.replace_instruction(ins_prev, conv_op, 
                    {ins_prev->arguments[0], l_weights});
