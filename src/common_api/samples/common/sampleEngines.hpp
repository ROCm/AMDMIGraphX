...
struct Parser
{
    std::unique_ptr<mgxonnxparser::IParser> onnxParser;

    operator bool() const { return onnxParser != nullptr; }
};

//!
//! \brief Helper struct to faciliate engine serialization and deserialization. It does not own the
//! underlying memory.
//!
struct EngineBlob
{
...
//!
//! \brief A helper class to hold a serialized engine (std or safe) and only deserialize it when
//! being accessed.
//!
class LazilyDeserializedEngine
{
    ...
    //!
    LazilyDeserializedEngine(bool isSafe,
                             bool versionCompatible,
                             int32_t DLACore,
                             std::string const& tempdir,
                             mgxinfer1::TempfileControlFlags tempfileControls,
                             std::string const& leanDLLPath)
        : mIsSafe(isSafe),
    ...
    //!
    mgxinfer1::ICudaEngine* get();

    //!
    //! \brief Get the pointer to the ICudaEngine and release the ownership.
    //!
    mgxinfer1::ICudaEngine* release();

    //!
    //! \brief Get the pointer to the safe::ICudaEngine. Triggers deserialization if not already
    //! done so.
    //!
    mgxinfer1::safe::ICudaEngine* getSafe();
    ...
    EngineBlob const getBlob() const
    {
        ...
        {
            return EngineBlob{const_cast<void*>(static_cast<void const*>(mEngineBlob.data())),
                              mEngineBlob.size()};
        }
    ...
    //!
    //! \brief Set the underlying blob storing the serialized engine without duplicating
    //! IHostMemory.
    //!
    void setBlob(std::unique_ptr<mgxinfer1::IHostMemory>& data)
    {
    ...
    //!
    //! \brief Set the underlying blob storing the serialized engine without duplicating vector
    //! memory.
    //!
    ...
    // Directly use the host memory of a serialized engine instead of duplicating the engine in CPU
    // memory.
    std::unique_ptr<mgxinfer1::IHostMemory> mEngineBlobHostMemory;

    std::string mTempdir{};
    mgxinfer1::TempfileControlFlags mTempfileControls{getTempfileControlDefaults()};
    ...
    std::unique_ptr<mgxinfer1::IRuntime> mParentRuntime{};

    //! The runtime that is used to deserialize the engine.
    std::unique_ptr<mgxinfer1::IRuntime> mRuntime{};

    //! If mIsSafe is false, this points to the deserialized std engine
    std::unique_ptr<mgxinfer1::ICudaEngine> mEngine{};

    //! If mIsSafe is true, this points to the deserialized safe engine
    std::unique_ptr<mgxinfer1::safe::ICudaEngine> mSafeEngine{};
...
bool setupNetworkAndConfig(const BuildOptions& build,
                           ...
                           mgxinfer1::IBuilder& builder,
                           mgxinfer1::INetworkDefinition& network,
                           mgxinfer1::IBuilderConfig& config,
                           std::ostream& err,
...
//!
void dumpRefittable(mgxinfer1::ICudaEngine& engine);
...
//!
mgxinfer1::ICudaEngine* loadEngine(std::string const& engine, int32_t DLACore, std::ostream& err);
...
//!
bool saveEngine(mgxinfer1::ICudaEngine const& engine,
...
//!
bool getEngineBuildEnv(ModelOptions const& model,
                       BuildOptions const& build,
                       SystemOptions& sys,
...
//!
mgxinfer1::IHostMemory* networkToSerialized(const BuildOptions& build,
                                            const SystemOptions& sys,
                                            mgxinfer1::IBuilder& builder,
                                            mgxinfer1::INetworkDefinition& network,
                                            std::ostream& err);
...
//! \return Pointer to a host memory for a serialized network
//!
mgxinfer1::IHostMemory* modelToSerialized(const ModelOptions& model,
...
//!
bool serializeAndSave(const ModelOptions& model,
                      const BuildOptions& build,
                      const SystemOptions& sys,
                      std::ostream& err);

bool timeRefit(const mgxinfer1::INetworkDefinition& network,
               mgxinfer1::ICudaEngine& engine,
               bool multiThreading);

//!
//! \brief Set tensor scales from a calibration table
//!
void setTensorScalesFromCalibration(mgxinfer1::INetworkDefinition& network,
                                    std::vector<IOFormat> const& inputFormats,
                                    std::vector<IOFormat> const& outputFormats,
                                    std::string const& calibrationFile);
...
//!
mgxinfer1::safe::IRuntime* createSafeInferRuntime(mgxinfer1::ILogger& logger) noexcept;
...
//!
mgxinfer1::consistency::IConsistencyChecker*
createConsistencyChecker(mgxinfer1::ILogger& logger, mgxinfer1::IHostMemory const* engine) noexcept;
...
bool loadStreamingEngineToBuildEnv(std::string const& engine,
                                   BuildEnvironment& env,
                                   std::ostream& err);

bool loadEngineToBuildEnv(std::string const& engine,
