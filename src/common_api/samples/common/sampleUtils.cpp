...
#if CUDA_VERSION >= 11060
void print(std::ostream& os, __hip_fp8_e4m3_fnuz v)
{
    os << static_cast<float>(v);
}
#endif
...
template void dumpBuffer<BFloat16>(void const* buffer, std::string const& separator, std::ostream& os, Dims const& dims,
    Dims const& strides, int32_t vectorDim, int32_t spv);
#if CUDA_VERSION >= 11060
template void dumpBuffer<__hip_fp8_e4m3_fnuz>(void const* buffer, std::string const& separator, std::ostream& os,
...
#if CUDA_VERSION >= 11060
template void fillBuffer<__hip_fp8_e4m3_fnuz>(void* buffer, int64_t volume, float min, float max);
#endif
