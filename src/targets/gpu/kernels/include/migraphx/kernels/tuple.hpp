...
struct tuple_storage<detail::seq<Ns...>, Ts...>
: element_storage<Ts, Ns>...
{
    template<class... Us, MIGRAPHX_REQUIRES(sizeof...(Us) == sizeof...(Ts))>
    constexpr tuple_storage(Us... ys)
    : element_storage<Ts, Ns>{ys}...
    {}
...
{ \
    ...
    return lhs([&](auto&... xs) { \
        return rhs([&](const auto&... ys) { \
            return result{xs op ys...}; \
        }); \
    }); \
}
