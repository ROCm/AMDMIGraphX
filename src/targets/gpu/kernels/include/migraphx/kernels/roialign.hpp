...
bilinear_interpolate(const T* data, const std::array<std::size_t, 2>& dims, std::array<float, 2> xy, Op pooling)
{
    std::array<int, 2> low{};
    ...
    std::array<std::size_t, 4> locs = {low[0] * dims[1] + low[1], low[0] * dims[1] + high[1], high[0] * dims[1] + low[1], high[0] * dims[1] + high[1]};
                          ...
                          Op op)
{
    ...
    dfor(bin_grid_size[0], bin_grid_size[1])([&](auto iy, auto ix) {
        ...
        locs[0] = roi_starts[0] + idx[0] * bin_size[0] + (iy + 0.5f) * bin_size[0] / bin_grid_size[0];
        locs[1] = roi_starts[1] + idx[1] * bin_size[1] + (ix + .5f) * bin_size[1] / bin_grid_size[1];
    ...
    make_tensors()(in_x, in_rois, in_ind, y)(
        [=](auto x_t, auto rois_t, auto ind_t, auto y_t) __device__ {
            ...
            for(index_int i = index.global; i < out_s.elements(); i += stride)
            {
                ...
                std::array<float, 2> roi_starts = {offset_rois[1] * spatial_scale, offset_rois[0] * spatial_scale};
                std::array<float, 2> roi_ends = {offset_rois[3] * spatial_scale, offset_rois[2] * spatial_scale};
