...
struct miopen_apply
{
    ...
    void add_loop_op()
    {
        apply_map.emplace("loop", [=](instruction_ref ins) {
            ...
            std::transform(copy_inputs.begin(), copy_inputs.end(), std::back_inserter(inputs), [&](auto in) {
                return mod->insert_instruction(ins, make_op("hip::allocate", {{"shape", to_value(in->get_shape())}}));
            });
