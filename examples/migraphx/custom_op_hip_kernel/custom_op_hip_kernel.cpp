...
struct square_custom_op final : migraphx::experimental_custom_op_base
{
    ...
    compute(migraphx::context ctx, migraphx::shape, migraphx::arguments inputs) const override
    {
        ...
        hipLaunchKernelGGL(vector_square, dim3(blocks), dim3(threadsPerBlock), 0, 0, d_output, d_output, 8192);
...
int main(int argc, const char* argv[])
{
    ...
    std::transform(expected_result.begin(), expected_result.end(), expected_result.begin(), [](auto i) {return std::pow(i, 2);});
